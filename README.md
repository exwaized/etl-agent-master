...
An autonomous data pipeline that detects its own failures, asks a local LLM to diagnose and patch them, tests the patch in an isolated Docker sandbox, and escalates to Slack only when it cannot recover on its own.
...
4. Runs the patch inside a locked-down Docker container (no network, 256 MB RAM)
5. If the patch exits cleanly, records the fix and marks the step succeeded
6. If the patch fails, retries the whole loop up to `MAX_ATTEMPTS` times
7. If all attempts fail, posts a structured alert to a Slack webhook and halts the run
...
Pipeline step raises an exception
   ...
   └─ response: { diagnosis, code_patch }
          │
          ▼
   sandbox.run_patch()                      (Docker)
   └─ docker run --network none
                 --memory 256m
                 --read-only
   └─ exit 0 → patch accepted
   └─ exit ≠ 0 or timeout → patch rejected
          │
     ┌────┴────┐
  accepted   rejected
     │            │
...
| Patch sandbox | Docker (`python:3.12-slim`, `--network none`, `--memory 256m`) |
...
│   └── sandbox.py             # Docker-based patch execution
├── api/
...
A run where `cast_types` fails (the CSV has a `revenue` column, not `amount`), the LLM proposes a fix, the sandbox validates it, and the pipeline resumes:
...
2024-11-14 09:12:04  INFO      __main__  patch applied successfully
...
- Apply the accepted patch back to the source file rather than discarding it after the run, so the fix persists across runs
